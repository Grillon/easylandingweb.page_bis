...
import { PortfolioCustomizationOptions } from '../components/portfolio/PortfolioCustomizationMenu';
import { getPortfolioTemplateStyles } from './portfolioTemplateStyles';

interface PortfolioMedia {
  type: { isImage?: boolean; isVideo?: boolean; isIframe?: boolean; };
  url: string;
  titre: string;
  description: string;
}
...
interface PortfolioCVFile {
  url: string;
  titre: string;
  description: string;
}

interface PortfolioSocial {
  nom: string;
  url: string;
}
...
interface PortfolioData {
  ...
  customizationOptions?: PortfolioCustomizationOptions;
}
...
function getDocumentIcon(type: string): string {
  const icons: Record<string, string> = {
    ...
    'doc': `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z"/>
    </svg>`,
...
export function generatePortfolioHTML(data: PortfolioData): string {
  ...
  const normalizedData = {
    nom: data.nom || '',
    about: data.about || data.accroche || '',
    projets: data.projets || [],
    documents: data.documents || (data.cv?.fichiers || []).map(f => ({ nom: f.titre, url: f.url, type: '' })),
    reseaux: data.reseaux || (data.social || []).map(s => ({ nom: s.nom, url: s.url, icone: '', pseudo: '' })),
    galerie: data.galerie?.images || []
  };

  // Générer la galerie à partir des projets qui ont un visuel
  const galerie = normalizedData.projets
    .filter(projet => projet.visuel || (projet.medias && projet.medias.length > 0))
    .map(projet => ({
      image: projet.visuel || (projet.medias && projet.medias[0] ? projet.medias[0].url : ''),
      projet: projet.titre.replace(/\s+/g, '-').toLowerCase()
    }));

  // Classer les documents par type
  const documentsClasses = normalizedData.documents.map(doc => {
...
<head>
  ...
  <title>Portfolio de ${normalizedData.nom}</title>
...
<body>
  ...
  <header class="header">
    <div class="container">
      <h1 class="fade-in">Portfolio de ${normalizedData.nom}</h1>
      ${normalizedData.about ? `<p class="text-xl opacity-90 mt-4">${normalizedData.about}</p>` : ''}
    </div>
  </header>
  ...
  <div class="container">
    ...
    <section id="about" class="section fade-in">
      <div class="card">
        <div class="projet">
          ...
          <p class="text-lg leading-relaxed text-center max-w-4xl mx-auto">${normalizedData.about || 'Développeur passionné par la création de solutions innovantes.'}</p>
        </div>
      </div>
    </section>
    ...
    <section id="projets" class="section fade-in">
      ...
      ${normalizedData.projets.length > 0 ? normalizedData.projets.map(projet => `
      <div class="card projet" id="${projet.titre.replace(/\s+/g, '-').toLowerCase()}">
        <h3 class="text-2xl font-bold mb-4">${projet.titre}</h3>
        
        ${projet.type || projet.annee || projet.stack || projet.role ? `
        <div class="projet-meta">
          ${projet.type ? `<div><strong>Type :</strong> ${projet.type}</div>` : ''}
          ${projet.annee ? `<div><strong>Année :</strong> ${projet.annee}</div>` : ''}
          ${projet.stack ? `<div><strong>Stack :</strong> ${projet.stack}</div>` : ''}
          ${projet.role ? `<div><strong>Rôle :</strong> ${projet.role}</div>` : ''}
        </div>
        ` : ''}
        
        ${projet.description ? `
        <div class="mb-4">
          <p class="text-lg">${projet.description}</p>
        </div>
        ` : ''}
        ...
        ${projet.solution && projet.solution.length > 0 ? `
        <div class="mb-4">
        ...
        ${projet.apprentissage && projet.apprentissage.length > 0 ? `
        <div class="mb-4">
        ...
        <div class="mt-6">
          <p><strong class="text-lg">Médias :</strong></p>
          <div class="documents-grid mt-3">
            ${projet.medias.map(media => `
            <div class="document-item">
              <div class="document-icon">${media.type.isImage ? getDocumentIcon('image') : media.type.isVideo ? getDocumentIcon('video') : getDocumentIcon('link')}</div>
              <div class="flex-1">
                <div class="font-medium">${media.titre}</div>
                <div class="document-type">${media.type.isImage ? 'Image' : media.type.isVideo ? 'Vidéo' : 'Lien'}</div>
                ${media.description ? `<div class="text-sm text-gray-600">${media.description}</div>` : ''}
              </div>
            </div>
            `).join('')}
          </div>
        ...
        <div class="mt-6">
          <p><strong class="text-lg">Ressources :</strong></p>
          <div class="documents-grid mt-3">
  ...
  <footer class="footer">
    <div class="container">
      ${normalizedData.reseaux.length > 0 ? `
      <div class="socials">
        ${normalizedData.reseaux.map(reseau => `
      ...
      <div class="mt-8 pt-8 border-t border-gray-600">
        ...
        <p class="text-xs opacity-50 mt-2">© ${new Date().getFullYear()} ${normalizedData.nom}. Tous droits réservés.</p>
      </div>
    </div>
  </footer>
